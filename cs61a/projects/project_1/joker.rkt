...
(define (best-total-for n)
  ...
  (define (is-card? c)
    (lambda (x) (eq? c x)))
  (define (not-one-nor-other? p1 p2)
    (lambda (x) (not (or (p1 x) (p2 x)))))
  (define (min-ace-value t aaj) 1)
  (define (min-joker-value current-total as-and-js)
    (let ((diff-to-n (- n current-total (dec as-and-js))))
      (if (> diff-to-n 0) diff-to-n 1)))
  (define (variable-value min-value)
    (lambda (r current-total as-and-js)
      (if (> (+ current-total 11 (- as-and-js 1)) n)
          (min-value current-total as-and-js)
          11)))
  (define (fixed-value rank current-total aaj)
    (if (integer? rank) rank 10))
  (define (sum-cards cards v-fn current-total as-and-js aaj-fn)
    (if (empty? cards)
        current-total
        (let ((rank (bl (first cards))))
          (sum-cards (bf cards)
                    v-fn
                    (+ current-total (v-fn rank current-total as-and-js))
                    (aaj-fn as-and-js)
                    aaj-fn))))
  (lambda (hand)
    (let ((fixed-value-cards (filter (not-one-nor-other? (is-rank? 'A) (is-card? 'JOKER)) hand))
          (as (filter (is-rank? 'A) hand))
          (js (filter (is-card? 'JOKER) hand)))
      (debug "fixed" fixed-value-cards)
      (debug "as" as) ;; this (js) is not being filtered correctly (probably symbol vs. string issue).
      (debug "js" js) ;; I'm considering that's done though - it's been too long :)
      (sum-cards js
                (variable-value min-joker-value)
                (sum-cards as
                            (variable-value min-ace-value)
                            (sum-cards fixed-value-cards
                                      fixed-value
                                      0
                                      0
                                      identity)
